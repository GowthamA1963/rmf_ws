...
const getTaskLocation = (task: TaskState): string => {
  if (!task.phases) {
    return 'unknown';
  }
  const phases = Object.values(task.phases);
  const firstPhase = phases[0]?.detail;
  let startWaypoint = '';
  if (firstPhase) {
    ...
    if (colonIndex > -1 && closingBracketIndex > -1 && closingBracketIndex > colonIndex) {
      startWaypoint = firstPhase.toString().substring(colonIndex + 1, closingBracketIndex);
    }
  }

  const lastPhase = phases[phases.length - 1]?.detail;
  let finishWaypoint = '';
  if (lastPhase) {
    finishWaypoint = lastPhase.toString().replace(/^.+:/, '').replace(/.$/, '');
  }

  if (startWaypoint && finishWaypoint) {
    return `${startWaypoint} -> ${finishWaypoint}`;
  }
  return finishWaypoint || startWaypoint || 'unknown';
};
...
export function TaskDataGridTable({
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      width: 200,
      editable: false,
      valueGetter: (params: GridValueGetterParams) => getTaskLocation(params.row as unknown as TaskState),
      flex: 1,
      filterable: false,
      sortable: false,
    },
